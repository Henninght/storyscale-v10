// Shared prompt builder for Preview and Generate APIs
// Ensures 100% consistency between preview and final draft generation
...
export const styleExamples: Record<string, string> = {
...
export function getExampleForStyle(
  style: string,
  tone: string,
  purpose?: string,
  length?: string,
 ...
 */
export function getContextAwareLength(
  length: string,
  purpose?: string,
  style?: string
): string {
...
/**
 * Build unified system prompt for both Preview and Generate APIs
 * @param profile - User profile data
 * @param wizardSettings - Wizard settings from request
 * @param recentPosts - Recent posts for duplication detection (empty for preview)
 * @param antiDuplication - Whether to enable anti-duplication mode
 */
...
/**
 * Build user message for both Preview and Generate APIs
 * @param wizardSettings - Wizard settings from request
 * @param referenceContent - Fetched reference URL content
 */
